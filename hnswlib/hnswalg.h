...
namespace hnswlib {
    ...
    public:
        ...
        std::priority_queue<std::pair<dist_t, labeltype >> searchKnn(void *query_data, size_t k) {
            ...
            std::priority_queue<std::pair<dist_t, tableint>, vector<pair<dist_t, tableint>>, CompareByFirst> top_candidates = searchBaseLayerSTInflu(
                    k1_rez.second, query_data, k);
            std::priority_queue<std::pair<dist_t, labeltype >> results2;
            while (top_candidates.size() > k) {
                top_candidates.pop();
            }
            ...
            while (top_candidates.size() > 0) {
                ...
                results2.push(std::pair<dist_t, labeltype>(rez.first, getExternalLabel(rez.second)));
                top_candidates.pop();
            }
            return results2;
        };
